...
struct CaloriesWidgetView: View {
    ...
    var body: some View {
        ...
        case .accessoryCircular:    CaloriesCircularView(data: entry.data)
        case .accessoryRectangular: CaloriesRectangularView(data: entry.data)
...
struct CaloriesCircularView: View {
    ...
    var body: some View {
        Gauge(value: data.calorieFraction) {
...
struct CaloriesRectangularView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
            Label("Calories Today", systemImage: "flame.fill")
                .font(.caption).widgetAccentable()
            Text("\(Int(data.calories)) / \(Int(data.calorieGoal)) kcal")
                .font(.headline)
            Text("P \(Int(data.protein))g  ·  C \(Int(data.carbs))g  ·  F \(Int(data.fat))g")
